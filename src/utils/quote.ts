...
export async function displayQuote(options: Options) {
  ...
  if (!isValidNetwork(network)) {
    console.error(
      `Error: Unsupported network "${network}". Supported networks are: ${Object.keys(
        ammPoolsPerChain
      ).join(", ")}.`
    )
    return
  ...
  if (!isValidContractType(contractType)) {
    console.error(
      `Error: Unsupported contract type "${contractType}". Supported contract types are: standard-token, tax-token, deflationary-token, test-erc20, my-custom-contract.`
    )
    return
  }
  ...
  const quoteData: QuoteData[] = availablePools.map(pool => ({
    ...
    lendingPoolApprovalCost:
      pool === "Xchange"
        ? calculateLendingPoolApprovalCost(gasPrice, contractType)
        : "N/A",
    loanInitiationCost:
      pool === "Xchange"
        ? calculateLoanInitiationCost(gasPrice, contractType)
        : "N/A",
    deploymentCostUsd: usd
      ? calculateDeploymentCost(gasPrice, contractType) * nativeTokenPrice
      : undefined,
    pairCreationCostUsd: usd
      ? calculatePairCreationCost(gasPrice, contractType) * nativeTokenPrice
      : undefined,
    lendingPoolApprovalCostUsd:
      pool === "Xchange" && usd
        ? calculateLendingPoolApprovalCost(gasPrice, contractType) *
          nativeTokenPrice
        : "N/A",
    loanInitiationCostUsd:
      pool === "Xchange" && usd
        ? calculateLoanInitiationCost(gasPrice, contractType) * nativeTokenPrice
        : "N/A",
  }))
...
function calculatePairCreationCost(
  gasPrice: number,
  contractType: string
): number {
...
function calculateLendingPoolApprovalCost(
  gasPrice: number,
  contractType: string
): number {
...
function calculateLoanInitiationCost(
  gasPrice: number,
  contractType: string
): number {
...
function formatPrice(
  ...
  return isUsd
    ? formatUsdPrice(price as number)
    : formatNativePrice(price as number, network)
}
