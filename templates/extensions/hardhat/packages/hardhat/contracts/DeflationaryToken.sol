/**
    DeflationaryToken Contract
    The contract is a deflationary ERC20 token contract with auto-burn functionality. 
    It includes features like transaction fees, liquidity generation, and manual/automatic 
    burning of liquidity pair tokens.
...
interface IAMMV2Router02 is IAMMV2Router01 {
...
contract DeflationaryToken is ERC20, Ownable {
    ...
    IAMMV2Router02 public immutable xchangeV2Router;
    address public immutable ammV2Pair;
    ...
    event UpdateXchangeV2Router(address indexed newAddress, address indexed oldAddress);
...
constructor() ERC20("DeflationaryToken", "DT") {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != ammV2Pair, "The pair cannot be removed from automatedMarketMakerPairs");
        ...
        if(limitsInEffect){
            if (
                ...
                if (transferDelayEnabled){
                    if (to != owner() && to != address(xchangeV2Router) && to != address(ammV2Pair)){
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        xchangeV2Router.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    function autoBurnLiquidityPairTokens() internal returns (bool){
        ...
        uint256 liquidityPairBalance = this.balanceOf(ammV2Pair);
 
        uint256 amountToBurn = liquidityPairBalance.mul(percentForLPBurn).div(10000);
 
        if (amountToBurn > 0){
            super._transfer(ammV2Pair, address(0xdead), amountToBurn);
        }
 
        TokenPair pair = TokenPair(ammV2Pair);
        pair.sync();
        emit AutoNukeLP();
        return true;
    }
